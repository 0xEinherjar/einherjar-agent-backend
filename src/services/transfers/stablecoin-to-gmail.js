...
import { sendEmail } from "../../libraries/resend.js";
...
export class StablecoinToGmailService {
  constructor({ userRepository, walletProvider }) {
    ...
    try {
      ...
      if (!to.endsWith("@gmail.com")) {
        return left({
          success: false,
          type: "BAD_REQUEST",
          message: "Recipient must be a valid @gmail.com address",
        });
      }
      ...
      let recipient = await this.userRepository.loadOne({ gmailAddress: to });
      if (!recipient) {
      ...
      if (balance < amount) {
        if (input.token === "USDC") {
          ...
          const bridgeFeeBuffer = Number(Math.round(0.5 * 10 ** decimals));
      ...
      const htmlContent = buildTransferReceiptEmail({
        senderName:
          user.gmailAddress || user.twitterUsername || "Einherjar User",
        amount: input.value,
  ...
  async _tryBridgeFromOtherChains({
    ...
    if (remaining > 0) {
      const totalOtherChains = chainsWithBalance.reduce(
        (sum, b) => sum + b.balance,
        0,
      );
      return left({
